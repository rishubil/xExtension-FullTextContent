...
[[ -x "${TEST_DEPS_DIR}/bin/obscura" ]]              || { echo "ERROR: ${TEST_DEPS_DIR}/bin/obscura missing" >&2; exit 1; }
[[ -f "${TEST_DEPS_DIR}/node_modules/defuddle/dist/cli.js" ]] || { echo "ERROR: defuddle missing" >&2; exit 1; }
[[ -f "${TEST_DEPS_DIR}/.versions" ]]                || { echo "ERROR: ${TEST_DEPS_DIR}/.versions missing" >&2; exit 1; }

# shellcheck source=/dev/null
source "${TEST_DEPS_DIR}/.versions"  # exports DEFUDDLE_PINNED_VERSION
export DEFUDDLE_PINNED_VERSION

echo "    obscura: $("${TEST_DEPS_DIR}/bin/obscura" --help 2>&1 | head --lines=1)"
echo "    node:    $(node --version)"
echo "    defuddle pinned to: ${DEFUDDLE_PINNED_VERSION}"
INSTALLED_DEFUDDLE=$(sed -n 's/.*"version": *"\([^"]*\)".*/\1/p' "${TEST_DEPS_DIR}/node_modules/defuddle/package.json" | head --lines=1)
echo "    defuddle installed: ${INSTALLED_DEFUDDLE}"
if [[ "${INSTALLED_DEFUDDLE}" != "${DEFUDDLE_PINNED_VERSION}" ]]; then
    echo "ERROR: defuddle version mismatch (image=${INSTALLED_DEFUDDLE}, expected=${DEFUDDLE_PINNED_VERSION})" >&2
    exit 1
fi
...
echo "============================================================"
PIPELINE_EXIT=0
php "${EXT}/tests/integration/test_pipeline.php" || PIPELINE_EXIT=$?
...
echo "============================================================"
FRESHRSS_EXIT=0
php "${EXT}/tests/integration/test_freshrss.php" || FRESHRSS_EXIT=$?
...
if [[ "${PIPELINE_EXIT}" -eq 0 ]] && [[ "${FRESHRSS_EXIT}" -eq 0 ]]; then
    ...
    echo "=== INTEGRATION TESTS FAILED (pipeline=${PIPELINE_EXIT}, freshrss=${FRESHRSS_EXIT}) ===" >&2
    exit 1
fi
